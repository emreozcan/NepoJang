...
from paths import ROOT_PRIVATE_KEY, ROOT_PUBLIC_KEY, ROOT_CERTIFICATE


def create_and_write_root_certificate(overwrite=False):
    if not overwrite and ROOT_CERTIFICATE.exists() and ROOT_PRIVATE_KEY.exists() and ROOT_PUBLIC_KEY.exists():
        return
    ...
    with open(ROOT_PRIVATE_KEY, "wb") as root_key_file:
    ...
    with open(ROOT_PUBLIC_KEY, "wb") as root_pub_file:
    ...
    with open(ROOT_CERTIFICATE, "wb") as root_crt_file:
